...
require_once(JPATH_SITE . DS . 'administrator' . DS . 'components' . DS . 'com_intotimeline' . DS . 'libraries' . DS . 'adodb' . DS . 'adodb-time.inc.php');
$componentPath = JURI::base() . 'components/com_intotimeline/';
?>
...
if ($task == 'add') {
    ...
    if ($user && $user->id && JRequest::getString('modifiedby') == $user->id) {
        ...
        echo 'Event Saved! Go to <a href="' . JURI::root() . 'index.php?option=com_intotimeline&timelineid=' . $post['timeline_id'] . '">timeline</a>';
        return;
    } else {
        echo 'Restricted access. Please login';
        return;
...
if ($timelineId) {
    ...
    if ($timeline->query) {
        require_once(JPATH_SITE . DS . 'administrator' . DS . 'components' . DS . 'com_intotimeline' . DS . 'controller.php');
        IntoTimelineController::generateTimelineFile($timelineId);
    }
    ...
    if (!$timeline->published) {
        ...
        if ($settings->display_description) {
            echo '<span class="_tl_description">' . $timeline->description . '</span>';
        }

        $db = & JFactory::getDBO();
        if ($timelineId) {
            $bquery = 'SELECT * FROM #__tl_band WHERE timeline_id = ' . $timelineId;
            $db->setQuery($bquery);
        ...
        <script type='text/javascript' src='<?php echo $componentPath; ?>js/into_timeline.js'></script>
    ...
    if (count($rows)) {
        echo '<div class="_tl_timelines">';
        foreach ($rows as $row) {
            ?>
            <span class="_tl_timeline">
                <?php $link = intoGetLink('index.php?option=com_intotimeline&timelineid=' . $row->id); ?>
